...
(function() {
    ...
    // Capture activation minute for the reload logic...
    const activationMins = new Date().getMinutes();
    // Track the last successful reload hour...
    ...
    const fgIframe = document.createElement('iframe');
    fgIframe.src = fgURLp;
    fgIframe.setAttribute('scrolling', 'no');
    fgIframe.setAttribute('allowTransparency', 'true');
    Object.assign(fgIframe.style, {
        width: 'calc(100% + 20px)',
    ...
    fgLayer.appendChild(fgIframe);
    document.body.appendChild(fgLayer);

    // —5— (A) Scheduled Reload (if fgURLh > 0 — i.e. enabled)...
    if (fgURLh > 0) {
        // Create a hidden staging iframe to test reloads...
        const stagingIframe = document.createElement('iframe');
        stagingIframe.style.display = 'none';
        stagingIframe.src = 'about:blank';
        document.body.appendChild(stagingIframe);

        // For M_compare, use fgURLm, but if fgURLm is outside the range 0-59 (disabled), use the minute the script activated...
        let M_compare = fgURLm;
        if (fgURLm < 0 || fgURLm > 59) { M_compare = activationMins; }
        ...
        setInterval(() => {
            ...
            // Fire only if: correct interval hour AND correct minute AND hasn't fired this hour yet...
            if (nowHour % fgURLh === 0 && nowMins === M_compare && lastReloadHour !== nowHour) {
                console.log(`[Superimpose] Attempting staged reload of fgURLp at ${nowHour}:${nowMins}`);
                lastReloadHour = nowHour;

                // Define the onload listener BEFORE setting the src to ensure it catches the event
                stagingIframe.onload = function() {
                    // Guard: Prevent another swap if we just loaded the about:blank "neutralizer"...
                    try {
                        if (stagingIframe.contentWindow.location.href.includes('about:blank')) return;
                    } catch (e) {
                        // Catching potential CORS errors if checking the location of a cross-origin site.
                    }

                    // Do the swap...
                    console.log(`[Superimpose] Safe load successful. Swapping content.`);
                    fgIframe.src = fgURLp;

                    // Neutralize the stagingIframe to stop potentially heavy background resource usage...
                    stagingIframe.src = 'about:blank';
                };

                // Try to load in the background first...
                stagingIframe.src = fgURLp;
            }
